...
for i in $SYS; do
    ...
    env GOOS=$OS GOARCH=$ARCH go build -v -ldflags "$COMMITFLAGS" github.com/shelvenzhou/lnd
    env GOOS=$OS GOARCH=$ARCH go build -v -ldflags "$COMMITFLAGS" github.com/shelvenzhou/lnd/cmd/lncli
    cd ..
    if [[ $OS = "windows" ]]; then
